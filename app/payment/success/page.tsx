  ...
  function PaymentSuccessContent() {
    ...
    try {
      ...
      if (orderId?.startsWith('test_order_')) {
        ...
        console.log('✅ Pending purchase loaded 1:', purchase)
      ...
      // Get current user for later linking
      ...
      if (currentUser) {
        ...
        console.log('ℹ️ User not logged in during verification')
      }
      ...
      console.log('✅ Pending purchase loaded:', purchase)
      ...
      // ✅ NEW: If user is logged in and purchase not linked, link it now
      if (currentUser && !purchase.accountLinked) {
        console.log('🔗 Linking purchase to current user:', currentUser.uid)
        try {
          const linkResult = await linkPurchaseToUser(orderId!, currentUser.uid)
          if (linkResult.success) {
            console.log('✅ Purchase linked successfully!')
            // Redirect directly to purchases page
            router.push('/profile/purchases')
            setLoading(false)
            setVerifying(false)
            return // Exit early
          } else {
            console.error('❌ Failed to link:', linkResult.error)
          }
        } catch (linkError) {
      ...
      if (!purchase.accountLinked) {
        ...
        console.log('✅ Purchase already linked, redirecting')
        router.push('/profile/purchases')
      }
    ...
    try {
      ...
      // R5: Auto-link this purchase (even before email verification)
      await linkPurchaseToUser(orderId!, userCred.user.uid)

      console.log('✅ Account created, purchase linked, and verification email sent')
